...
/* ---------- UI: abrir/cerrar panel índice ---------- */
let showNavPanel = true;
function setNavPanelVisible(v){
  showNavPanel = !!v;
  const panel = document.getElementById("navPanel");
  if(panel) panel.style.display = showNavPanel ? "" : "none";
}
...
function initMap3D(){
  ...
  if(btnToggleNav) btnToggleNav.addEventListener("click", () => setNavPanelVisible(!showNavPanel));

  // UI: botones que deben funcionar en 3D
  ...
  if(!THREE){
    console.warn("[3D] Three.js no está cargado (window.THREE).");
    setHint3D("3D desactivado: no se pudo cargar Three.js. (Se usa modo 2D/SVG)");
    return false;
  }
  ...
  if(!gl){
    console.warn("[3D] WebGL no disponible en este navegador.");
    setHint3D("3D desactivado: WebGL no disponible/bloqueado en el navegador. (Se usa modo 2D/SVG)");
    return false;
  }

  document.body.classList.add("mode3d");
  const svgFallback = document.getElementById("space");
  if(svgFallback) svgFallback.style.display = "none";
  // Marca modo 3D activo (para debug/UI)
  window.__mode3d = true;
  setHint3D("3D activo — Arrastra: rotar · Shift/Click derecho: pan · Rueda: zoom · Click: seleccionar");

  const scene = new THREE.Scene();
  scene.fog = new THREE.FogExp2(0xe7f2ff, 0.00035);

  const camera = new THREE.PerspectiveCamera(55, 1, 0.1, 20000);
  camera.position.set(0, -1250, 780);

  let renderer;
  try{
    renderer = new THREE.WebGLRenderer({ canvas, antialias: true, alpha: true, powerPreference: "high-performance" });
  }catch(err){
    console.warn("[3D] Falló WebGLRenderer:", err);
    setHint3D("3D desactivado: el navegador impidió inicializar WebGLRenderer. (Se usa modo 2D/SVG)");
    return false;
  }
  renderer.setPixelRatio(Math.min(2, window.devicePixelRatio || 1));
  renderer.setClearColor(0x000000, 0);

  // "Editor look": rejilla + ejes para dar referencia 3D
  const grid = new THREE.GridHelper(8000, 80, 0x2d89ff, 0xb7c9de);
  ...
  scene.add(grid);
  const axes = new THREE.AxesHelper(260);
  axes.position.set(0, 0, 2);
  scene.add(axes);

  // Controles tipo "editor 3D" (sin OrbitControls)
  const controls = {
    target: new THREE.Vector3(0, 0, 0),
    distance: 1600,
    minDistance: 180,
    maxDistance: 6000,
    theta: Math.PI * 0.15, // yaw
    phi: Math.PI * 0.38,   // pitch
    rotSpeed: 0.006,
    panSpeed: 0.95,
    zoomSpeed: 0.0012,
  };
  ...
  function updateCamera(){
    controls.phi = clamp(controls.phi, 0.08, Math.PI - 0.08);
    controls.distance = clamp(controls.distance, controls.minDistance, controls.maxDistance);

    const sinPhi = Math.sin(controls.phi);
    const x = controls.distance * sinPhi * Math.cos(controls.theta);
    const y = controls.distance * sinPhi * Math.sin(controls.theta);
    const z = controls.distance * Math.cos(controls.phi);
  ...
  canvas.addEventListener("pointerdown", (e) => {
    ...
    lastX = e.clientX;
    lastY = e.clientY;
    if(e.button === 0 && !e.shiftKey) isRotating = true;
    else isPanning = true; // botón derecho o Shift+drag = pan
  });
  canvas.addEventListener("pointermove", (e) => {
    ...
    if(isRotating){
      ...
      const vRight = new THREE.Vector3().crossVectors(vDir, camera.up).normalize();
      const vUp = new THREE.Vector3().copy(camera.up).normalize();
      const panScale = controls.panSpeed * (controls.distance / 1200);
      controls.target.addScaledVector(vRight, -dx * panScale);
      controls.target.addScaledVector(vUp, dy * panScale);
      updateCamera();
    }
  });
  ...
  canvas.addEventListener("wheel", (e) => {
    e.preventDefault();
    controls.distance *= (1 + (e.deltaY * controls.zoomSpeed));
    updateCamera();
  }, { passive: false });

  scene.add(new THREE.AmbientLight(0xffffff, 0.85));
  const key = new THREE.DirectionalLight(0xbfe7ff, 0.9);
  key.position.set(700, -600, 900);
  scene.add(key);
  const rim = new THREE.DirectionalLight(0x66ffd1, 0.35);
  rim.position.set(-900, 900, 500);
  scene.add(rim);
  const fill = new THREE.DirectionalLight(0xffd39a, 0.25);
  fill.position.set(200, 1200, 300);
  scene.add(fill);
  ...
  function kindColor(kind){
    switch(kind){
      case "C": return 0x2d89ff;
      case "L1": return 0xff8a1f;
      case "L2": return 0x6a5cff;
      case "L3": return 0x2aa184;
      case "L4": return 0xff4a4a;
  ...
  function computeDepthZ(){
    const depthById = new Map();
    function depthOf(id){
      if(depthById.has(id)) return depthById.get(id);
      const n = nodeById.get(id);
      if(!n || !n.parent) { depthById.set(id, 0); return 0; }
      const d = depthOf(n.parent) + 1;
      depthById.set(id, d);
      return d;
    }
    ...
    for(const n of nodes){
      ...
      // Más separación para que se note el 3D
      n._zBase = -d * 140;
      n._z = n._zBase;
    }
  }

  function assignOrbitsAndPhases3D(){
    for(const [parentId, childIds] of childrenByParent.entries()){
      ...
      childIds.forEach((id, i) => {
        ...
        // inclinación estable para órbitas 3D (no todo plano)
        const u = hashUnit(c.id);
        c._tilt = (u - 0.5) * 0.9; // -0.45..0.45
      });
    }
  }

  function computePositions3D(t){
    // centros (x,y fijos)
    for(const n of nodes){
    ...
    // resolver jerarquía por pasadas (como el engine D3)
    for(let pass=0; pass<6; pass++){
      for(const n of nodes){
        ...
        // órbita en XY + inclinación en Z (3D)
        const dx = Math.cos(a) * R;
        const dy = Math.sin(a) * R;
        const tilt = n._tilt ?? 0;
        const dz = Math.sin(a) * R * tilt * 0.22;
  ...
  function buildGasStationMesh(scale){
    const g = new THREE.Group();

    const baseMat = new THREE.MeshStandardMaterial({ color: 0x0a1a24, metalness: 0.6, roughness: 0.35 });
    const glowMat = new THREE.MeshStandardMaterial({ color: 0x1affd8, metalness: 0.2, roughness: 0.2, emissive: 0x0a5a4a, emissiveIntensity: 1.3 });
    const whiteMat = new THREE.MeshStandardMaterial({ color: 0xf2f6ff, metalness: 0.2, roughness: 0.25 });

    const ring = new THREE.Mesh(new THREE.TorusGeometry(28, 3.2, 10, 40), glowMat);
    ring.rotation.x = Math.PI / 2.1;
    g.add(ring);

    const core = new THREE.Mesh(new THREE.CylinderGeometry(10, 12, 40, 16), baseMat);
    core.position.z = 8;
    g.add(core);

    const canopy = new THREE.Mesh(new THREE.CylinderGeometry(26, 26, 6, 24), whiteMat);
    canopy.position.z = 30;
    g.add(canopy);

    const nozzle = new THREE.Mesh(new THREE.BoxGeometry(10, 4, 8), baseMat);
    nozzle.position.set(18, 0, 18);
    g.add(nozzle);

    const led = new THREE.Mesh(new THREE.BoxGeometry(18, 3.2, 3.2), glowMat);
    led.position.set(0, 0, 30);
    g.add(led);
  ...
  function rebuildScene(){
    ...
    buildChildrenIndex();
    computeDepthZ();
    assignOrbitsAndPhases3D();
    computePositions3D(0);

    const netMat = new THREE.LineBasicMaterial({ color: 0x6affeb, transparent: true, opacity: 0.62 });
    const softMat = new THREE.LineDashedMaterial({ color: 0x9cffd8, transparent: true, opacity: 0.46, dashSize: 18, gapSize: 10 });

    for(const e of edges){
      const a = nodeById.get(e.s);
      const b = nodeById.get(e.t);
      if(!a || !b) continue;
      const positions = new Float32Array(6);
      positions[0] = (a._x ?? a.x ?? 0);
      positions[1] = (a._y ?? a.y ?? 0);
      positions[2] = (a._z ?? 0);
      positions[3] = (b._x ?? b.x ?? 0);
      positions[4] = (b._y ?? b.y ?? 0);
      positions[5] = (b._z ?? 0);
      const geom = new THREE.BufferGeometry();
      geom.setAttribute("position", new THREE.BufferAttribute(positions, 3));
      const mat = (e.type === "soft") ? softMat : netMat;
      const line = new THREE.Line(geom, mat);
      if(line.computeLineDistances) line.computeLineDistances();
      edgesGroup.add(line);
      edgeLines.push({ line, e });
    }

    for(const n of nodes){
      const pos = new THREE.Vector3(n._x ?? n.x ?? 0, n._y ?? n.y ?? 0, n._z ?? 0);
      if(n.model === "gasStation"){
        const m = buildGasStationMesh(Math.max(0.9, (n.r ?? 44) / 55));
        m.position.copy(pos);
      ...
      const radius = Math.max(10, (n.r ?? 44) * 0.34);
  ...
  function rebuildLabels(){
    ...
    for(const n of nodes){
      ...
      el.innerHTML = `<span class="t3d"></span><span class="sub3d"></span>`;
      labelsLayer.appendChild(el);
      labelElById.set(n.id, el);
    }
  }
  ...
  function updateLabels(){
    if(!labelsLayer) return;
    if(!showLabels3D){
      for(const el of labelElById.values()) el.style.display = "none";
      return;
    }
    ...
    for(const n of nodes){
      const el = labelElById.get(n.id);
      if(!el) continue;

      v.set(n._x ?? n.x ?? 0, n._y ?? n.y ?? 0, n._z ?? 0).project(camera);
      const inFront = v.z < 1;
      ...
      const x = (v.x * 0.5 + 0.5) * width + rect.left;
      const y = (-v.y * 0.5 + 0.5) * height + rect.top;
      el.style.left = x + "px";
      el.style.top = y + "px";

      const title = el.querySelector(".t3d");
      const sub = el.querySelector(".sub3d");
      if(title) title.textContent = n.label || n.id;
      if(sub){
        sub.textContent = showLabelDescs3D ? (n.desc || "") : "";
        sub.style.display = (showLabelDescs3D && n.desc) ? "" : "none";
      }
  ...
  function setBreadcrumb3D(){
    const crumb = document.getElementById("crumb");
    if(crumb) crumb.textContent = "Ruta: " + (WORLDS[worldKey]?.name ?? worldKey);
  }

  function updateBackState(){
    if(!btnBack) return;
    const canBack = history3D.length > 1;
    if(canBack) btnBack.removeAttribute("disabled");
    else btnBack.setAttribute("disabled", "true");
  }

  function setWorld(key, { push } = { push: true }){
    worldKey = key;
    currentWorldKey = key;
    nodes = (WORLDS[key]?.nodes ?? []).map(n => ({ ...n }));
    edges = (WORLDS[key]?.edges ?? []).map(e => ({ ...e }));
    ...
    window.__worldById = new Map(nodes.map(n => [n.id, n]));
    if(window.refreshNavPanel) window.refreshNavPanel();
  ...
  function stepWorld(tms){
    // tiempo en segundos desde el arranque (consistente con 2D)
    const t = tms * 0.001;
    computePositions3D(t);

    // actualiza meshes
    for(const n of nodes){
      const mesh = meshById.get(n.id);
      if(!mesh) continue;
      mesh.position.set(n._x ?? n.x ?? 0, n._y ?? n.y ?? 0, n._z ?? 0);
    }

    // actualiza edges
    for(const { line, e } of edgeLines){
      const a = nodeById.get(e.s);
      const b = nodeById.get(e.t);
      if(!a || !b) continue;
      const attr = line.geometry.getAttribute("position");
      attr.setXYZ(0, a._x ?? a.x ?? 0, a._y ?? a.y ?? 0, a._z ?? 0);
      attr.setXYZ(1, b._x ?? b.x ?? 0, b._y ?? b.y ?? 0, b._z ?? 0);
      attr.needsUpdate = true;
      if(line.computeLineDistances) line.computeLineDistances();
    }
  }

  function highlightNode3D(id){
    for(const [nid, mesh] of meshById.entries()){
      mesh.traverse?.((o) => {
        if(!o.material) return;
        const mats = Array.isArray(o.material) ? o.material : [o.material];
        for(const mat of mats){
          if(mat.emissive) mat.emissive.setHex(nid === id ? 0x3cffd8 : 0x000000);
          if(typeof mat.emissiveIntensity === "number") mat.emissiveIntensity = nid === id ? 1.15 : 0.0;
        }
      });
    }
  }
  ...
  canvas.addEventListener("mousemove", (ev) => {
    const nodeId = pickNodeId(ev);
    if(!nodeId){ tipHide(); return; }
    const n = window.__worldById?.get(nodeId);
    if(!n){ tipHide(); return; }
    tipShow(`${n.label}${n.desc ? " — " + n.desc : ""}`, ev.clientX, ev.clientY);
  }, { passive: true });

  canvas.addEventListener("mouseleave", () => tipHide(), { passive: true });
  ...
  canvas.addEventListener("click", (ev) => {
    ...
    navSelectedNodeId = n.id;
    if(window.refreshNavPanel) window.refreshNavPanel();
    highlightNode3D(n.id);

    const shift = ev.shiftKey;
    if(n.world && !shift){
      setWorld(n.world, { push: true });
      requestAnimationFrame(() => {
        focusNode("L4_core");
        highlightNode3D("L4_core");
      });
      return;
    }

    if(n.clickable === false) return;

    if(n.url){
      if(shift) window.open(n.url, "_blank");
      else OPEN_IN_NEW_TAB ? window.open(n.url, "_blank") : (window.location.href = n.url);
    }else{
      focusNode(n.id);
    }
  });
  ...
  // integración con el índice (gotoNodeById usa estas funciones si existen)
  window.highlightNode = (id) => highlightNode3D(id);
  window.focusWorldPoint = (x, y) => {
    controls.target.set(x, y, 0);
    updateCamera();
  };
  window.loadWorld = (key, opts) => setWorld(key, { push: opts?.push !== false });

  buildNavPanel();
  setNavPanelVisible(true);
  setWorld("main", { push: false });

  // Botones 3D: etiquetas / conexiones / volver
  if(btnToggleEdges){
    btnToggleEdges.addEventListener("click", () => {
      showEdges3D = !showEdges3D;
      edgesGroup.visible = showEdges3D;
    });
  }
  if(btnToggleLabels){
  ...
  if(btnBack){
    btnBack.addEventListener("click", () => {
      if(history3D.length <= 1) return;
      history3D.pop();
      const prev = history3D[history3D.length - 1] || "main";
      setWorld(prev, { push: false });
    });
    updateBackState();
  }
  ...
  function animate(tms){
    ...
    updateLabels();
    // camera already updated by input handlers
...
// NavPanel toggle (D3 mode)
if(btnToggleNav){
  btnToggleNav.addEventListener("click", () => {
    showNav = !showNav;
    setNavPanelVisible(showNav);
  });
}
...
window.addEventListener("resize", () => {
  ...
  // En modo 3D, apaga el SVG por completo por seguridad visual.
  const svgEl = document.getElementById("space");
